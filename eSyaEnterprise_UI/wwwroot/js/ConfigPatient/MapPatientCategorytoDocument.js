            ...
            if (prevSelectedID != data.node.id) {
                ...
                if (data.node.id == "0") {
                    ...
                    if (data.node.parent == "#") {
                        ...
                        $('#' + data.node.id + "_anchor").html($('#' + data.node.id + "_anchor").html() + '<span id="Edit" style="padding-left:5px;padding-right:5px">&nbsp;<i class="fa fa-pen" style="color:#555555"aria-hidden="true"></i></span>')
                        $('#' + data.node.id + "_anchor").html($('#' + data.node.id + "_anchor").html() + '<span id="View" style="padding-left:5px;padding-right:5px">&nbsp;<i class="fa fa-eye" style="color:#555555"aria-hidden="true"></i></span>')
                        ...
                        $('#Edit').on('click', function () {
                            if (_userFormRole.IsEdit === false) {
                                fnAlert("w", "EPM_03_00", "UIC02", errorMsg.editauth_E2);
                                return;
                            }
                            $('#pnlMapPatientCategoryDocument').hide();
                            ...
                            $("#btnAddMapPatientCategoryDocument").show();
                            $("#btnAddMapPatientCategoryDocument").html('<i class="fa fa-sync"></i> ' + localization.Update);
                            __IsAddClicked = 0; __IsEditClicked = 1; __IsViewClicked = 0;
                        });

                        $('#View').on('click', function () {
                            if (_userFormRole.IsView === false) {
                                fnAlert("w", "EPM_03_00", "UIC03", errorMsg.vieweauth_E3);
                                return;
                            }
                            $('#pnlMapPatientCategoryDocument').hide();
                            ...
                            $("#btnAddMapPatientCategoryDocument").hide();
                            $("#btnAddMapPatientCategoryDocument").html('<i class="fa fa-sync"></i> ' + localization.Update);
