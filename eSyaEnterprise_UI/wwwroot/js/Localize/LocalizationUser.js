...
function validateUser() {
    ...
    if ($("#txtLoginId").val().trim().length <= 0) {
        fnAlert("w", "ELE_06_00", "UI0094", errorMsg.LoginID_E6);
        return false;
    }
    if ($("#txtLoginDescription").val().trim().length <= 0) {
        fnAlert("w", "ELE_06_00", "UI0095", errorMsg.LoginDesc_E7);
        return false;
    }
    if ($("#txtPassword").val().trim().length <= 0) {
        fnAlert("w", "ELE_06_00", "UI0096", errorMsg.Password_E8);
        return false;
    }

    if (!PasswordPattern.test($("#txtPassword").val())) {
        fnAlert("w", "ELE_06_00", "UI0097", errorMsg.PasswordRule_E9);
        return false;
    }
}
