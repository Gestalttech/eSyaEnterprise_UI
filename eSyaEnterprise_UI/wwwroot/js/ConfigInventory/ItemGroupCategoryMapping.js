...
function fnItemGroupCateSubCateMapping() {
    ...
    if (cboitemgroup == "0" || cboitemgroup == null || cboitemgroup == undefined) {
        ...
        fnAlert("w", "ECS_07_00", "UI0189", errorMsg.ItemSubCategory_E8);
        return false;
    }
    else {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.Status == true) {
                    if (flag == "1") {
                        fnAlert("s", "ECS_07_00", response.StatusCode, response.Message);
                    }
                    else if (flag == "0") {
                        fnAlert("s", "ECS_07_00", response.StatusCode, response.Message);
                        $("#cboitemgroup").val('0');
                    ...
                    fnAlert("e", "ECS_07_00", response.StatusCode, response.Message);
                }
                $("#btnIGCAdd").attr("disabled", false);
            },
            error: function (error) {
                fnAlert("e", "ECS_07_00", error.StatusCode, error.statusText);
                $("#btnIGCAdd").attr("disabled", false);
            }
        });
    }
