...
function fnGridLoadSMSVariable() {
    ...
    $("#jqgSMSVariable").jqGrid({
        url: getBaseURL() + '/Notification/GetSMSVariableInformation',
        datatype: 'json',
        mtype: 'Post',
...
function fnEditSMSVariable(e, actiontype) {
    ...
    if (actiontype.trim() == "edit") {
        if (_userFormRole.IsEdit === false) {
            fnAlert("w", "EPS_23_00", "UIC02", errorMsg.editauth_E2);
            return;
        }
        $("#PopupSMSVariable").modal('show');
    ...
    if (actiontype.trim() == "view") {
        if (_userFormRole.IsView === false) {
            fnAlert("w", "EPS_23_00", "UIC03", errorMsg.vieweauth_E3);
            return;
        }
        $("#PopupSMSVariable").modal('show');
    ...
    if (actiontype.trim() == "delete") {
        if (_userFormRole.IsDelete === false) {
            fnAlert("w", "EPS_23_00", "UIC04", errorMsg.deleteauth_E4);
            return;
        }
        $("#PopupSMSVariable").modal('show');
...
function fnSaveSMSVariable() {
    ...
    if (IsStringNullorEmpty($("#txtSMSVariable").val())) {
        fnAlert("w", "EPS_23_00", "UI0098", errorMsg.NotificationVariable_E6);
        return false;
    }
    if (IsStringNullorEmpty($("#txtSMSComponent").val())) {
        fnAlert("w", "EPS_23_00", "UI0099", errorMsg.NotificationComponent_E7);
        return false;
    }
    ...
    var URL = getBaseURL() + '/Notification/InsertIntoSMSVariable';
    if (isUpdate == 1) {
        URL = getBaseURL() + '/Notification/UpdateSMSVariable';
    }
...
function fnDeleteSMSVariable() {
    ...
    $.ajax({
        url: getBaseURL() + '/Notification/ActiveOrDeActiveSMSVariable?status=' + a_status + '&smsvariable=' + $("#txtSMSVariable").val(),
        type: 'POST',
        success: function (response) {
            if (response.Status) {
                fnAlert("s", "", response.StatusCode, response.Message);
