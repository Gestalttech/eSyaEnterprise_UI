...
function fnLoadGridAreaController() {
    ...
    $("#jqgAreaController").jqGrid({
        ...
        autowidth: true,
        shrinkToFit: true,
        forceFit: true, caption:localization.AreaController,
        loadComplete: function (data) {
            SetGridControlByAction();
            fnJqgridSmallScreen("jqgAreaController");
        },
...
function fnEditAreaController(e, actiontype) {
    ...
    if (actiontype.trim() == "delete") {
        ...
        if (rowData.ActiveStatus == 'true') {
            $("#btndeActiveAreaController").html(localization.Deactivate);
        }
        else {
 ...
 function fnSaveAreaController() {
    ...
    if (IsStringNullorEmpty($("#txtArea").val())) {
        fnAlert("w", "EPS_02_00", "UI0005", errorMsg.AreaName_E4);
        return;
    }
    if (IsStringNullorEmpty($("#txtController").val())) {
        fnAlert("w", "EPS_02_00", "UI0006", errorMsg.ControllerName_E5);
        return;
    }
    obj_area = {
