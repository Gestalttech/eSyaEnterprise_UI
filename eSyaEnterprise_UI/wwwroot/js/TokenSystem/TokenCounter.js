...
function fnGridLoadTokenCounter() {
    ...
    $("#jqgTokenCounter").jqGrid({
        ...
        shrinkToFit: true,
        forceFit: true,
        caption: localization.TokenCounter,
...
function fnAddTokenCounter() {
    if (IsStringNullorEmpty($("#cboBusinessLocation").val()) || $("#cboBusinessLocation").val() === "0") {
        ...
        _isInsert = true;
        $("#cboFloor").next().attr('disabled', false).selectpicker('refresh');
    }
}
...
function fnEditTokenCounter(e, actiontype) {
    ...
    $("#cboFloor").next().attr('disabled', true).selectpicker('refresh');
...
function fnDeleteTokenCounter() {
    ...
    $.ajax({
        url: getBaseURL() + '/CounterMapping/ActiveOrDeActiveTokenCounter?status=' + a_status + '&businesskey=' + $("#cboBusinessLocation").val() + '&counternumber=' + $("#txtCounterNumber").val() + '&floorId=' + $("#cboFloor").val(),
        type: 'POST',
        success: function (response) {
            if (response.Status) {
                fnAlert("s", "", response.StatusCode, response.Message);
