...
namespace eSyaEnterprise_UI.Areas.TokenSystem.Controllers
{
    public class GenerateController : Controller
    {
        ...
        public GenerateController(IeSyaTokenSystemAPIServices eSyaTokenSystemAPIServices, ISmsServices smsServices, ILogger<GenerateController> logger)
        {
        ...
        public async Task<JsonResult> ConfirmOTP(DO_OTP obj)
        {
            try
            {
                var serviceResponse = await _eSyaTokenSystemAPIServices.HttpClientServices.PostAsJsonAsync<DO_ReturnParameter>("TokenGeneration/ConfirmOTP", obj);
                if (serviceResponse.Status)
                {
                    if (serviceResponse.Data.Status)
        ...
        public async Task<JsonResult> GetTokenDetailByMobile(int businessKey, int isdCode, string mobileNumber)
        {
            try
            {
                ...
                var serviceResponse = await _eSyaTokenSystemAPIServices.HttpClientServices.GetAsync<List<DO_Token>>("TokenManagement/GetTokenDetailByMobile" + parameter);
                if (serviceResponse.Status)
                    return Json(serviceResponse.Data);
                else
                    return Json(new DO_ReturnParameter() { Status = false, Message = serviceResponse.Message });
