...
function validateUserMaster() {
    ...
    if ($("#txtLoginId").val().trim().length <= 0) {
        fnAlert("w", "EEU_05_00", "UI0133", errorMsg.LoginID_E1);
        return false;
    }
    if ($("#txtLoginDescription").val().trim().length <= 0) {
        fnAlert("w", "EEU_05_00", "UI0134", errorMsg.LoginDesc_E2);
        return false;
    }
    if ($("#txtReason").val().trim().length <= 0) {
        fnAlert("w", "EEU_05_00", "UI0135", errorMsg.ReasonForDeactivation_E3);
        return false;
    }
}
