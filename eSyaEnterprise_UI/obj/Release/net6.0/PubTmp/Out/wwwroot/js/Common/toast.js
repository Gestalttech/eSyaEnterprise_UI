...
(function ($) {
    ...
    const TOAST_CONTAINER_HTML = `<div aria-live="polite" aria-atomic="true" class="position-relative">
                                    <div id="toast-container" class="toast-container position-fixed top-0 start-50 translate-middle-x p-1">
                                    </div>
                                 </div>`;
    // (2) Set some defaults
    $.toastDefaults = {
    ...
    function render(opts) {
        ...
        switch (type) {
            ...
            case 'secondary':
                ...
                break;
            case 'success':
                classes.header.bg = $.toastDefaults.style.success || 'er-bg-success';
                classes.header.fg = $.toastDefaults.style.info || 'text-white';
                break;
            case 'error':
            case 'danger':
                classes.header.bg = $.toastDefaults.style.error || 'er-bg-danger';
                classes.header.fg = $.toastDefaults.style.error || 'text-white';
                break;
            case 'warning':
                classes.header.bg = $.toastDefaults.style.warning || 'er-bg-warning';
        ...
        if (title) {
            ...
            if (subtitle) {
                _subtitle = `<small class="fs-6 fw-bold">${subtitle}</small>`;
            }
            if (img) {
            ...
            if (content) {
                if (classes.header.bg == "er-bg-success") {
                    html += `<div class="toast-body text-wrap d-flex flex-row">
                           <i class="fa-regular fa-circle-check fa-2xl me-2"></i> <span class="text-justify"> ${content}  </span>
                         </div>`;
                }
                if (classes.header.bg == "er-bg-warning") {
                    html += `<div class="toast-body text-wrap d-flex flex-row">
                           <i class="fa-solid fa-circle-exclamation fa-xl me-2"></i>  <span class="text-justify"> ${content}   </span>
                         </div>`;
                }
                if (classes.header.bg == "er-bg-danger") {
                    html += `<div class="toast-body text-wrap d-flex flex-row">
                           <i class="fa-regular fa-circle-xmark fa-2xl me-2"></i> <span class="text-justify"> ${content}   </span>
                         </div>`;
                }
            }
            html += `</div>`;
